...
paymentsList(ProviderData provider, List<Widget> list, Function() _redraw, double price,
    MainModel mainModel, Function() book2){
  ...
  dec(String image, int type){
    return InkWell(
        onTap: (){
          paymentMethod = type;
          String _desc = "";
          paymentProcess(price, _desc, waitInMainWindow, buildContext, _book2, _mainModel);
        },
        child: Container(
  ...
  if (provider.acceptPaymentInCash)
    list.add(dec("assets/cache.png", 1));
  ...
  var _paymentItems = [
    PaymentItem(
      label: 'Total',
      amount: price.toStringAsFixed(appSettings.digitsAfterComma),
