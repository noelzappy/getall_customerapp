...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance!.addPostFrameCallback((_) {
      ...
      if (t != 0)
        _scrollController2.animateTo(t,
  ...
  _body() {
    ...
    list.add(Row(
      children: [
        Expanded(flex: 3, child: comboBoxAddress(_mainModel)),
        SizedBox(width: 10),
        Expanded(
          flex: 1,
          child: button2(strings.get(222), theme.mainColor, () {
            /// "Nearby you",
            route("nearby");
          }),
        ),
    ...
    for (var item in appSettings.customerAppElements) {
        ...
        for (var item in categories) {
          ...
          if (t != null) {
            list.add(Container(
              ...
              width: windowSize,
              height: 60,
              child: button157(getTextByLocale(item.name, strings.locale),
                  item.color, item.serverPath, () {}, windowSize, 60),
            )));
            list.add(t);
            list.add(SizedBox(
              height: 10,
      ...
      if (item == "related_products") {
        ...
        list.add(
            articleHorizontalBar("root", windowWidth, context, _mainModel));
      }
  ...
  _listFavorites(List<Widget> list) {
    ...
    for (var item in product) {
      if (!userAccountData.userFavorites.contains(item.id)) continue;
  ...
  _addTopServices(List<Widget> list) {
    for (var item in product) {
      if (!appSettings.inMainScreenServices.contains(item.id)) continue;
  ...
  Widget? _horizontalCategoryDetails(CategoryData parent) {
    ...
    var index = 0;
    for (var item in product) {
      if (!item.category.contains(parent.id)) continue;
      var _tag = UniqueKey().toString();
      list.add(Hero(
          tag: _tag,
          child: Container(
